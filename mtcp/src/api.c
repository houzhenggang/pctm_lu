...
/*----------------------------------------------------------------------------*/
int 
mtcp_connect_with_method(mctx_t mctx, int sockid, 
		const struct sockaddr *addr, socklen_t addrlen, uint8_t stream_method)
{
	mtcp_manager_t mtcp;
	socket_map_t socket;
	tcp_stream *cur_stream;
	...
	if (stream_method == 1) {
		cur_stream->stream_method = stream_method;
	}
	cur_stream->state = TCP_ST_SYN_SENT;
	TRACE_STATE("Stream %d: TCP_ST_SYN_SENT\n", cur_stream->id);
